...
describe('Spell model', () => {
  ...
  it('updates a row by id', async() => {
    ...
    const updatedSpell = await Spell.update(createdSpell.id, {
      name: 'wish',
      school: 'conjuration',
      level: 9
    });

    expect(updatedSpell).toEqual({
      id: createdSpell.id,
      name: 'wish',
      school: 'conjuration',
      level: 9
  ...
  it('deletes a row by id', async() => {
    ...
    const deletedSpell = await Spell.delete(createdSpell.id);

    expect(deletedSpell).toEqual({
      id: createdSpell.id,
      name: 'wish',
      school: 'conjuration',
      level: 9
